...
asiadf = gapminder %>% filter(region == "South-Eastern Asia") %>%
  filter(year>=1965) %>% filter(year<=2010) %>% select(country, year, gdp, population) %>% arrange(country, year)
...
#2
df = df %>% mutate(births = (fertility * womenpop / 30))

#3
df = df %>% arrange(country,year) %>% mutate(popchangeraw = (population - lag(population))) %>% mutate(newcountryflag = (country == lag(country))) %>% mutate(popchange = popchangeraw)
## this works because the last year for each country has missing population. If this was not the case, we would need to deal with the fact that, when the country changes, there is a 
...
table2 = tab %>% .[2] %>% html_table(fill = TRUE) %>% .[[1]]
#deaths_df = tab %>% .[3] %>% html_table %>% .[[1]]
head(table2)
```
...
```{r}
colnames(table2)
tableclean = table2 %>% select('Country or Territory', 4)
head(tableclean)
```
...
tableclean = tableclean %>% rename(country = 'Country or Territory')
