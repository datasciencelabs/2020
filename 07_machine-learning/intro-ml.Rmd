...
```
Let's look at the confusion matrix to see how well we are performing.
```{r}
...
Now that we know how to perform Naive Bayes by hand and understand the math behind it, we can use the `naiveBayes` function from the `e1071` package to do these calculations automatically. We see we get almost the exact same confusion matrix we got before we changed the prevalence. There may be some numerical rounding that results in 1 less male predicition using this function. 

```{r}
nb_fit   <- naiveBayes(sex ~ height, data = train_set)
p_hat_nb <- predict(nb_fit, test_set)
y_hat_nb <- ifelse(p_hat_logit > 0.5, "Female", "Male")
