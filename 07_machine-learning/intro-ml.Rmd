...
Now that we know how to perform Naive Bayes by hand and understand the math behind it, we can use the `naiveBayes` function from the `e1071` package to do these calculations automatically. We see we get almost the exact same confusion matrix we got before we changed the prevalence. There may be some numerical rounding that results in 1 less male prediction using this function. 
