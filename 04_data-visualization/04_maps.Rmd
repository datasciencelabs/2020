...
```{r}
theme_set(theme_bw())
multiple_countries_map <- map_data("world", c("japan", "singapore", "australia", "germany"))
...
But this is quite difficult to see. A better alternative would be to use `world2` data and specify `usa`:
...
`world2` centers the plot on the Pacific Ocean, allowing us to finally see Hawaii (barely) and bigger versions of Alaska and the mainland states (also known as the lower 48).
...
```{r}
georgia_map <- georgia_map %>% mutate(manual_fill = cut(rate, 
                                                    breaks = c(-Inf, 0, 621, 1071, 1623, 2960, Inf),
                                                    labels = c("None", "1-620", "621-1,070", "1,071-1,622", 
                                                               "1,623-2,960", ">2,960"), 
                                                    right = TRUE))
...
```{r}
ggplot(georgia_map, aes(x = long, y = lat, group = group)) +
